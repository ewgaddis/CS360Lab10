...
app.controller('shoppingController', [
	...
	function($scope, $http, $window) {
		...
		$http.get('/products/get').success(function(data, status, headers, config) {
			$scope.products = data;
		...
		$http.get('/customers/get').success(function(data, status, headers, config) {
			$scope.customer = data;
		}).error(function(data, status, headers, config) {
			$scope.customer = [];
		});
		
		$http.get('/orders/get').success(function(data, status, headers, config) {
			$scope.orders = data;
		}).error(function(data, status, headers, config) {
			$scope.orders = [];
		});
		...
		$scope.addToCart = function(productId) {
			...
			if(this.product.instock <= 0) {
				$window.alert('The product is unavailable.');
				return;
			}
			...
			for(var i = 0; i < $scope.customer.cart.length; i++) {
				...
				if(item.product[0]._id == productId) {
					if(item.product[0].instock <= item.quantity) {
						$window.alert('Cannot exceed available product.');
						return;
					}
		...
		$scope.verifyBilling = function(ccv) {
			...
			$http.post('/customers/update/billing', {
				updatedBilling: $scope.customer.billing[0],
				ccv:            ccv
			}).success(function(data, status, headers, config) {
